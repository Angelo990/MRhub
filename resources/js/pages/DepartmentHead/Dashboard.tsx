...
import { FileDown, FileSpreadsheet, FileText, FilePlus, Pencil, Printer, RotateCcw, Settings2, Download, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function Dashboard() {
    ...
    const chartCards = useMemo(() => [
        ...
        },
        {
            ...
            description: 'Request submission volume for the most recent months in your department.',
            className: '',
            content: (
                <div className="h-[340px] w-full">
                    <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
        ...
        },
        {
            ...
            description: 'Items most frequently requested by your department.',
                ...
                <div className="space-y-3">
                    {recentRequests.length > 0 ? recentRequests.map((request) => (
                        ...
                        <div className="rounded-lg border border-border/70 bg-background/60 p-4 text-sm text-muted-foreground">
                            No recent requests found for your department yet.
                        </div>
                    )}
                </div>
            ),
    ...
    const cardExportData: Record<string, () => Record<string, string | number>[]> = {
        ...
        'recent-requests': () => recentRequests.map((r) => ({ ID: r.id, Date: r.date, Purpose: r.purpose, Status: r.status, Items: r.itemCount, 'Total Qty': r.totalQuantity, 'Est. Value': r.estimatedValue })),
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${departmentName} Analytics Dashboard`} />
